...
module Boj
    ...
    def batch(testcases)
      ...
      for testcase in testcases
        ...
        block = lambda { |testcase|
          status, output, error, time = nil, nil, nil, nil
          Open3.popen3("java #{Boj.config["stage_path"]}/solution.java") do |i, o, e, t|
            ...
            status = result.success? ? 
              (verify_output(testcase, output) ? :success : :fail) : :error
          end
  ...
  private
    ...
    def verify_output(testcase, output)
      ...
      if answer_lines.size != output_lines.size
        return false
      end

      (0...answer_lines.size).each do |i|
        if answer_lines[i].chomp.rstrip != output_lines[i].chomp.rstrip
          return false
        end
      end
